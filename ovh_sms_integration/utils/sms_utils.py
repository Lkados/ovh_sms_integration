...
def send_sms(
    ...
    try:
        ...
        # Envoi du SMS
        result: "SMSResult" = settings.send_sms(
            formatted_message, validated_receiver, sender
        )
        return result
...
def validate_phone_number(phone: str | None) -> str:
    ...
    if not re.match(r"^\+\d{10,15}$", cleaned_phone):
        raise ValueError(
            _("Numéro de téléphone invalide: {0}. Format attendu: +33XXXXXXXXX").format(
                phone
            )
        )
...
def get_contact_mobile(doc: "Document") -> str | None:
    ...
    if hasattr(doc, "contact_person") and doc.contact_person:
        try:
            ...
            frappe.log_error(
                _("Erreur récupération contact {0}: {1}").format(
                    doc.contact_person, str(e)
                )
            )
...
def send_event_reminder_sms(
    ...
    try:
        ...
        message = format_event_reminder_message(
            template, event_doc, recipient_name, recipient_type
        )
        ...
        if result and result.get("success"):
            log_event_reminder_sent(
                event_doc.name, recipient_name, recipient_type, recipient_mobile
            )
            return result
...
def get_event_participants_with_mobile(event_name: str) -> list["EventParticipant"]:
    ...
    try:
        ...
        frappe.log_error(
            f"Erreur récupération participants événement {event_name}: {e}"
        )
        return []
            ...
            try:
                ...
                for participant in participants:
                    ...
                    if (
                        ...
                        and reminder_settings.send_to_customer_only
                    ):
                        should_send = True
                    elif (
                        participant["type"] == "employee"
                        and reminder_settings.send_to_employee
                    ):
                        should_send = True
                    elif (
                        not reminder_settings.send_to_customer_only
                        and not reminder_settings.send_to_employee
                    ):
                ...
                frappe.log_error(
                    f"Erreur traitement événement {event_data['name']}: {e}"
                )
                total_failed += 1
...
def update_reminder_statistics(sent_count: int, failed_count: int) -> None:
    ...
    try:
        ...
        today = now.date()
        last_check = (
            reminder_settings.last_check_time.date()
...
def send_manual_sms(
    ...
    for receiver in receivers:
        ...
        results.append(
            {"receiver": receiver, "success": result is not None, "result": result}
        )
