...
class SMSPricingItem(Document):
    ...
    def get_item_valuation_rate(self, item_code: str) -> float:
        """Récupère le taux de valorisation le plus récent d'un article.

        Args:
            item_code: Code de l'article à valoriser.

        Returns:
            float: Taux de valorisation ou 0 si non trouvé.
        """
        try:
    ...
    def validate_eur_currency(self):
        ...
        if self.currency != "EUR":
            frappe.msgprint(
                _("Attention: La devise doit être EUR pour les campagnes SMS")
            )
            self.currency = "EUR"
