...
class TestSMSEventReminder(unittest.TestCase):
        ...
        try:
            ...
            if not frappe.db.exists("Employee", {"employee_name": "Test Employee SMS"}):
                ...
                employees = frappe.get_all(
                    "Employee", filters={"employee_name": "Test Employee SMS"}, limit=1
                )
                if employees:
    ...
    def cleanup_test_data(self):
        ...
        try:
            ...
            if self.test_customer and frappe.db.exists(
            ...
            if self.test_employee and frappe.db.exists(
    ...
    def test_reminder_times_parsing(self):
        ...
        self.assertEqual(
            times, expected_times, "Parsing des heures de rappel incorrect"
        )
    ...
    def test_message_template_formatting(self):
        ...
        formatted_message = self.reminder_settings.format_message(
            template, event, customer_name="Test Customer"
        )

        self.assertIn("Test entretien SMS", formatted_message)
        self.assertIn("Test Customer", formatted_message)
        self.assertNotIn(
            "{{", formatted_message, "Variables non remplacées dans le template"
        )
    ...
    def test_event_filtering(self):
        ...
        _event_entretien = self.create_test_event(24)  # noqa: F841 Dans 24h
    ...
    def test_business_hours_check(self):
        ...
        with patch(
            "ovh_sms_integration.ovh_sms_integration.doctype.sms_event_reminder.sms_event_reminder.datetime"
        ) as mock_datetime:
            # Test pendant les heures ouvrables (14h un mardi)
            mock_datetime.now.return_value = datetime(
                2024, 1, 16, 14, 0, 0
            )  # Mardi 14h
            mock_datetime.now.return_value.time.return_value = datetime(
                2024, 1, 16, 14, 0, 0
            ).time()
            mock_datetime.now.return_value.weekday.return_value = 1  # Mardi

            self.assertTrue(
                self.reminder_settings.should_send_now(),
                "Devrait envoyer pendant les heures ouvrables",
            )
            ...
            self.assertFalse(
                self.reminder_settings.should_send_now(),
                "Ne devrait pas envoyer hors heures ouvrables",
            )
    ...
    def test_contact_mobile_extraction(self):
        ...
        self.assertEqual(
            mobile, "+33123456789", "Mobile client non extrait correctement"
        )
        ...
        if self.test_employee:
            ...
            self.assertEqual(
                mobile, "+33987654321", "Mobile employé non extrait correctement"
            )

    @patch(
        "ovh_sms_integration.ovh_sms_integration.doctype.sms_event_reminder.sms_event_reminder.send_sms"
    )
    def test_sms_sending_mock(self, mock_send_sms):
        ...
        _event = self.create_test_event()  # noqa: F841

        # Test d'envoi
        result = self.reminder_settings.send_sms_reminder(
            "Message de test", "+33123456789"
        )
